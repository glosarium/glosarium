            ...
            <form method="post" action="{{ route('contact.post') }}">
                ...
                <div class="col-sm-12 col-md-8">
                    ...
                    <div class="form-group {{ !$errors->has('subject') ?: 'has-error' }}">
                        <input type="text" name="subject" class="form-control input-lg" placeholder="Subjek pesan" value="{{ old('subject') }}">
                        <span class="help-block">{{ $errors->first('subject') }}</span>
                    </div>

                    <div class="form-group {{ !$errors->has('email') ?: 'has-error' }}">
                        @if (auth()->check())
                            <p class="form-control-static">Dikirim oleh: <strong>{{ auth()->user()->name }}</strong></p>
                        @else
                            <input type="email" name="email" class="form-control input-lg" placeholder="Alamat pos-el" value="{{ old('email') }}">
                        @endif
